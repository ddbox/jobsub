...
class JobSettings(object):
    ...
    def makeWrapFile(self):
        """ Make middle part of wrapfile
        ($CONDOR_TMP/user_job_(numbers)_wrap.sh . Execute
	...
	# Should we have f.close() corresponding to line 1205?
        ...
        f.close()
        iret, ires = ju.getstatusoutput(
            "cat %s/xtract.sh %s/%s.tgz > %s/%s.sh" % (tmpdir, tmpdir,
                                                       base, tmpdir, base))
        if iret:
            raise Exception(ires)
    ...
    def makeCondorFiles(self):
        ...
        if settings['usedagman'] == False:
            ...
            if settings['project_name'] == "":
                settings[
                    'project_name'] = "%s-%s" % (settings['user'],
                                                 settings['filetag'])
    ...
    def makeDAGFile(self):
        ...
        if settings['mail_summary']:
            f.write("SCRIPT POST %s_END %s %s \n" %
                    (jobname, settings['summary_script'],
                     settings['dagendfile']))
