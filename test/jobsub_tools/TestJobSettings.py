...
class JobTest(unittest.TestCase):
    ...
    def testConstructor(self):
        ...
        self.assertEqual(ns.settings['notify'].lower(), 'error')
    ...
    def testGoodInput(self):
        ...
        self.assertEqual(
            ns.settings['accountinggroup'],
            'thats_a_silly_group_name')
        ...
        self.assertEqual(ns.settings['notify'].lower(), 'never')
    ...
    def testMakingDagFiles(self):
        ...
        self.assertEqual(
            os.path.isfile(
                ns.settings['dagbeginfile']),
            True,
            ns.settings['dagbeginfile'])
        self.assertEqual(
            os.path.isfile(
                ns.settings['dagendfile']),
            True,
            ns.settings['dagendfile'])

        (retVal, output) = commands.getstatusoutput(
            "grep RunOnGrid  %s" % ns.settings['cmdfile'])
        self.assertEqual(
            retVal, 0, "file %s did not contain 'RunOnGrid' " %
            ns.settings['cmdfile'])
        ...
        self.assertEqual(
            retVal, 0, "command '%s' should have exited with status 0, got %s instead" %
            (cmd, retVal))
        self.assertEqual(
            output, response, "expected '%s' from '%s', got '%s' instead" %
            (output, cmd, response))
        #del ns
    ...
    def testMakingCommandFiles(self):
        ...
        ns.settings['accountinggroup'] = "group_w"
        ns.settings['exe_script'] = ns.__class__.__name__ + \
            "_MakeCommandFiles.sh"
        ...
        (retVal, output) = commands.getstatusoutput(
            "grep 'queue 1' %s" % ns.settings['cmdfile'])
        self.assertEqual(retVal, 0)
    ...
    def testTimeoutFlag(self):
        ...
        self.stdioON()
        (retVal, output) = commands.getstatusoutput(
            "grep '^timeout 5m $JOBSUB_EXE_SCRIPT'  %s" % ns.settings['wrapfile'])
    ...
    def testExpectedLifetimeFlag(self):
        ...
        self.assertEqual(
            retVal,
            0,
            'JOB_EXPECTED_MAX_LIFETIME not in ' +
            ns.settings['cmdfile'])
    ...
    def testOSFlag(self):
        ...
        (retVal, output) = commands.getstatusoutput(cmd)
        self.assertTrue(
            'stringlistimember(Target.IFOS_installed,DesiredOS)'.upper() in output.upper())

    def testResourceProvidesFlag(self):
        """test --resource-provides option"""
        ns = self.ns
        ns.runCmdParser(
            ['--resource-provides=usage_model=DEDICATED,OPPORTUNISTIC', 'some_script'])
        ns.checkSanity()
        self.assertEqual(
            ns.settings['resource_list'],
        ...
        (retVal, output) = commands.getstatusoutput(cmd)
        self.assertTrue(
            'stringListsIntersect(toUpper(target.HAS_usage_model), toUpper(my.DESIRED_usage_model)'.upper() in output.upper())
    ...
    def testSiteFlag(self):
        ...
        (retVal, output) = commands.getstatusoutput(cmd)
        self.assertTrue(
            'stringListIMember(target.GLIDEIN_Site,my.DESIRED_Sites)'.upper() in output.upper())
    ...
    def testBlacklistFlag(self):
        ...
        (retVal, output) = commands.getstatusoutput(cmd)
        self.assertTrue(
            'stringListIMember(target.GLIDEIN_Site,my.Blacklist_Sites)=?=false'.upper() in output.upper())

    def testCPNCommands(self):
        """test CPN i/o from -d and -f flags"""
        # jobsub -f input_file_1 -f input_file_2 -d FOO this_is_the_foo_dir -d
        # BAR this_is_the_bar_dir (some_subclass)_CPNtest.sh
        ns = self.ns
        ns.settings['input_dir_array'] = ['input_file_1', 'input_file_2']
        ns.settings['output_dir_array'] = [
            ('FOO', 'this_is_the_foo_dir'), ('BAR', 'this_is_the_bar_dir')]
        ns.settings['accountinggroup'] = "group_w"
        ...
        self.assertEqual(
            retVal,
            0,
            'cpn cant transfer out CONDOR_DIR_FOO in file ' +
            ns.settings['wrapfile'])

    def testGFTPCommands(self):
        """test gridFTP i/o from -d and -f flags"""
        # jobsub --use_gftp  -f input_file_1 -f input_file_2 -d FOO
        # this_is_the_foo_dir -d BAR this_is_the_bar_dir
        # (some_subclass)_GFTPtest.sh
        ns = self.ns
        ns.settings['input_dir_array'] = ['input_file_1', 'input_file_2']
        ns.settings['output_dir_array'] = [
            ('FOO', 'this_is_the_foo_dir'), ('BAR', 'this_is_the_bar_dir')]
        ...
        self.assertEqual(
            retVal,
            0,
            'gftp cant transfer out CONDOR_DIR_BAR in file ' +
            ns.settings['wrapfile'])
