...
scp jobsub_server/pkg/fnal-jobsub_server-0.0.1.tar.gz root@${REMOTE_HOST}:fnal-jobsub_server-0.0.1.tar.gz
echo "for MOD in \$(puppet module list | grep jobsub_server| awk '{print \$2}'); do" > $REMOTE_SCRIPT
echo "    puppet module uninstall \$MOD" >> $REMOTE_SCRIPT
echo "done" >> $REMOTE_SCRIPT
echo "puppet module install fnal-jobsub_server-0.0.1.tar.gz" >>$REMOTE_SCRIPT
